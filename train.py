...
def train(hyp, opt, device, tb_writer=None):
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, opt, hyp=hyp, augment=True,
                                            ...
                                            world_size=opt.world_size, workers=opt.workers,label_name = opt.label_name)
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(test_path, imgsz_test, total_batch_size, gs, opt, hyp=hyp, augment=False,
                                       ...
                                       workers=opt.workers,label_name = opt.label_name)[0]
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if save:
                ...
                if epoch%1 == 0:
                    torch.save(ckpt, wdir + ('epoch' + str(epoch) + '.pt'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='/home/industai/code_folder/Python_code/gitlab_project/yolov5-3.0/models/yolov5s.yaml', help='model.yaml path')
    ...
    # change logdir
    dataPath = Path(opt.data)
    dataName = dataPath.name.split('.yaml')[0]
    opt.logdir = opt.logdir + '/'

    # Set DDP variables
    opt.total_batch_size = opt.batch_size
    opt.world_size = int(os.environ['WORLD_SIZE']) if 'WORLD_SIZE' in os.environ else 1
    opt.global_rank = int(os.environ['RANK']) if 'RANK' in os.environ else -1
    print("opt.global_rank:{}".format(opt.global_rank))
