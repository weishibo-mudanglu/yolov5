...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8,label_name='txts'):
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, rank=-1,label_name='txts'):
        try:
            f = []  # image files
        ...
        # Define labels
        self.label_files = [x.replace('images', label_name).replace(os.path.splitext(x)[-1], '.txt') for x in
                            self.img_files]
        ...
        for i, file in pbar:
            ...
            if l is not None and l.shape[0]:
                ...
                if extract_bounding_boxes:
                    ...
                    img = cv2.imread(str(p)) #改成读灰度图测试单通道模型的训练
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        img = cv2.imread(path)  # BGR，改为读灰度图测试单通道模型的训练
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            ...
            # img4 = np.full((s * 2, s * 2), 114, dtype=np.uint8)  # base image with 4 tiles 测试单通道训练模型
